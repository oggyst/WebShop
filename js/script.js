...
function buy(element) {
	...
	function addProduct() {
		...
		if (strProductName == "" || intCost == 0) {
			...
			console.log(arrCategories[Math.floor(document.getElementById("st-categories").value)]);
	...
	function removeProduct ()
	{
		document.getElementById("st-select").innerHTML = "";
			...
			{
				console.log(dataProducts.value.length);
			}
		document.getElementById("st-content").innerHTML = "Chosse item you wanna remove:";
	}
						...
						for (var i = 1; i < data.value.length + 1; i++) {
							...
							for (var j = 0; j < products.value.length; j++) {
								if (i == products.value[j].CategoryID) {
									...
									products.value[j].QuantityPerUnit + "<input type='number' id='st-quantity" + products.value[j].ProductID + "'><button data-product-id=" + products.value[j].ProductID + " data-product-name=\"" +
									 products.value[j].ProductName + "\" data-unit-price=" + products.value[j].UnitPrice + " onclick='buy(this)'>Buy</button></div></div>";
								}
								if (boolDoOnce) {
									strContent += "<div class = 'st-product'>" + products.value[j].ProductName + "<br>" + Math.floor(products.value[j].UnitPrice) + "€ <div class = 'st-details'>" +
										products.value[j].QuantityPerUnit + "<input type='number' id='st-quantity" + products.value[j].ProductID + "'><button data-product-id=" + products.value[j].ProductID + " data-product-name=\"" +
										 products.value[j].ProductName + "\" data-unit-price=" + products.value[j].UnitPrice + " onclick='buy(this)'>Buy</button></div></div>";
								}
							}
